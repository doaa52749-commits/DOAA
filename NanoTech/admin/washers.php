...
if ($edit_id > 0) {
    $stmt = $pdo->prepare('SELECT * FROM washers WHERE washer_id = ?');
...
if (is_post()) {
    ...
    if ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        if ($id > 0) {
            $stmt = $pdo->prepare('DELETE FROM washers WHERE washer_id = ?');
            $stmt->execute([$id]);
        }
    ...
    try {
        $uploaded = save_uploaded_image('image_file', 'washers');
        if ($uploaded !== null) {
            $image_url = $uploaded;
        }
    } catch (Throwable $t) {
    ...
    if ($id > 0) {
        $stmt = $pdo->prepare('UPDATE washers SET type=?, version=?, size=?, color=?, price=?, quantity=?, image_url=? WHERE washer_id=?');
        $stmt->execute([$type, $version, $size, $color, $price, $quantity, $image_url, $id]);
    } else {
        $stmt = $pdo->prepare('INSERT INTO washers (type, version, size, color, price, quantity, image_url) VALUES (?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$type, $version, $size, $color, $price, $quantity, $image_url]);
    }

    redirect('washers.php');
}

$items = $pdo->query('SELECT * FROM washers ORDER BY washer_id DESC')->fetchAll();
...
<div class="row g-3">
  <div class="col-12 col-lg-5">
    <div class="card shadow-sm">
      <div class="card-body">
        <div class="h5 mb-3"><?= $editing ? 'تعديل مغسلة' : 'إضافة مغسلة' ?></div>
        <form method="post" enctype="multipart/form-data">
          <input type="hidden" name="id" value="<?= e((string)($editing['washer_id'] ?? 0)) ?>" />
          ...
          <div class="mb-2"><label class="form-label">الحجم</label><input class="form-control" name="size" value="<?= e((string)($editing['size'] ?? '')) ?>" required /></div>
          <div class="mb-2"><label class="form-label">اللون</label><input class="form-control" name="color" value="<?= e((string)($editing['color'] ?? '')) ?>" required /></div>
        ...
        <table class="table table-striped table-hover m-0">
          ...
          <tbody>
            <?php foreach ($items as $it): ?>
              <tr>
                <td><?= e((string)$it['washer_id']) ?></td>
                <td><?= e($it['type']) ?></td>
                <td><?= e($it['version']) ?></td>
                <td><?= e(number_format((float)$it['price'], 2)) ?></td>
                <td><?= e((string)$it['quantity']) ?></td>
                <td class="text-end">
                  <a class="btn btn-sm btn-outline-primary" href="washers.php?id=<?= e((string)$it['washer_id']) ?>">تعديل</a>
                  <form class="d-inline" method="post" onsubmit="return confirm('حذف العنصر؟');">
                    <input type="hidden" name="action" value="delete" />
                    <input type="hidden" name="id" value="<?= e((string)$it['washer_id']) ?>" />
